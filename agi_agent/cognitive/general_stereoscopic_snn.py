...
# -*- coding: utf-8 -*-
"""
通用立体脉冲神经网络（General Stereoscopic SNN）v3.0
...
class GeneralStereoscopicSNN:
    ...
    ┌────────┴──────────────────────────┴─────────────────────────────────┐
    │                      特征整合层                                      │
    │  模式整合(320神经元)    ←→    认知隐藏2层(50神经元)                  │
    └────────▲──────────────────────────▲─────────────────────────────────┘
             │                          │
    ┌────────┴──────────────────────────┴─────────────────────────────────┐
    │                      初级处理层                                      │
    │  特征提取(128) ←→ 缓冲整合(80)    ←→    认知隐藏1层(50神经元)        │
    └────────▲──────────────────────────▲─────────────────────────────────┘
             │                          │
    ┌────────┴──────────────────────────┴─────────────────────────────────┐
    │                      感知输入层                                      │
    │  感官编码(128通道)   ←→    认知输入层(60神经元)                      │
    └──────────────────────────────────────────────────────────────────────┘
    """
    ...
    def __init__(self, config: Dict = None):
        ...
        self._init_temporal_predictor()
        self._init_pattern_statistics()
    ...
    def _init_perceptual_path(self):
        from .hierarchical_snn import (
            SensoryEncoder, SensoryBuffer, FeatureExtractor,
            PatternIntegrator, HippocampalMemory, BasalGanglia
        )
        ...
        self.sensory_encoder = SensoryEncoder(self.num_channels, self.feature_dim)
        self.sensory_buffer = SensoryBuffer(self.num_channels)
        self.feature_extractor = FeatureExtractor(self.num_channels // 2, self.num_channels)
        self.pattern_integrator = PatternIntegrator(self.num_channels // 4)
        self.hippocampus = HippocampalMemory(500)
        self.basal_ganglia = BasalGanglia(300)

        self.perceptual_layer_sizes = {
            'sensory_encoder': self.num_channels,
            'sensory_buffer': self.num_channels,
            'feature_extractor': self.num_channels // 2 + 16,
            'pattern_integrator': (self.num_channels // 4) * 10,
            'hippocampus': 500,
            'basal_ganglia': 300
        }
        ...
        self.perceptual_layer_offsets = {
            'sensory_encoder': 0,
            'sensory_buffer': self.num_channels,
            'feature_extractor': self.num_channels + self.num_channels,
            'pattern_integrator': self.num_channels + self.num_channels + (self.num_channels // 2 + 16),
    ...
    def _build_cross_connections(self):
        layer_mappings = [
            ('sensory_encoder', 'input', ConnectionType.FEEDFORWARD, 1.0),
            ('sensory_buffer', 'input', ConnectionType.FEEDFORWARD, 1.2),
            ('feature_extractor', 'hidden1', ConnectionType.FEEDFORWARD, 1.5),
            ('pattern_integrator', 'hidden2', ConnectionType.FEEDFORWARD, 1.8),
            ('hippocampus', 'output', ConnectionType.FEEDFORWARD, 1.2),
            ('basal_ganglia', 'output', ConnectionType.FEEDFORWARD, 1.0),
            ('output', 'pattern_integrator', ConnectionType.FEEDBACK, 0.8),
            ('output', 'hippocampus', ConnectionType.FEEDBACK, 0.6),
            ('hidden2', 'feature_extractor', ConnectionType.FEEDBACK, 0.6),
            ('hidden1', 'sensory_buffer', ConnectionType.FEEDBACK, 0.5),
        ]
    ...
    def _init_intra_path_synapses(self):
        ...
        perceptual_layers = ['sensory_buffer', 'feature_extractor', 'pattern_integrator', 'hippocampus']
    ...
    def _init_temporal_predictor(self):
        self.temporal_neurons = 32
        self.temporal_phases = np.zeros(self.temporal_neurons)
        self.temporal_frequencies = np.logspace(0.5, 2.5, self.temporal_neurons)
        self.temporal_amplitudes = np.ones(self.temporal_neurons)
        self.temporal_input_weights = np.random.uniform(0.1, 0.3, (self.temporal_neurons, self.feature_dim))
        self.temporal_output_weights = np.random.uniform(0.1, 0.2, (self.feature_dim, self.temporal_neurons))
        
        self.event_predictions = deque(maxlen=100)
        self.current_temporal_scale = 1.0
        self.temporal_scale_history = deque(maxlen=50)
        
        self.event_strengths = np.zeros(8)
        self.event_index = 0

    def _update_temporal_predictor(self, current_t_ms: float, input_spikes: List[Any] = None):
        dt = self.dt / 1000.0
        
        input_influence = np.zeros(self.temporal_neurons)
        
        if input_spikes:
            for spike in input_spikes:
                neuron_idx = spike.channel % self.temporal_neurons if hasattr(spike, 'channel') else 0
                input_influence += self.temporal_input_weights[:, neuron_idx] * 0.5
        
        self.temporal_phases += 2 * np.pi * self.temporal_frequencies * dt * self.current_temporal_scale
        
        self.temporal_phases = self.temporal_phases % (2 * np.pi)
        
        oscillatory_activity = np.sin(self.temporal_phases) * self.temporal_amplitudes
        
        self.temporal_amplitudes = np.clip(
            self.temporal_amplitudes + input_influence * dt - 0.01 * dt,
            0.5, 2.0
        )
        
        event_spikes = []
        for i in range(self.temporal_neurons):
            if self.temporal_phases[i] < 0.1 and self.temporal_phases[i] > -0.1:
                event_spikes.append(i)
                self.event_index = (self.event_index + 1) % 8
                self.event_strengths[self.event_index] = self.temporal_amplitudes[i]
        
        if input_spikes and event_spikes:
            self.temporal_scale_history.append(self.current_temporal_scale)
            if len(self.temporal_scale_history) > 10:
                self.current_temporal_scale = np.mean(self.temporal_scale_history)
        
        return event_spikes

    def _init_pattern_statistics(self):
        self.pattern_weights = np.ones((self.feature_dim, self.feature_dim)) * 0.5
        self.feature_counts = np.zeros(self.feature_dim)
        self.transition_counts = np.zeros((self.feature_dim, self.feature_dim))
        self.central_feature_activation = np.zeros(self.feature_dim)
        
        self.current_pattern = 'default'
        self.pattern_confidence = 0.0
        self.pattern_strengths = np.zeros(self.feature_dim)
        self.transition_history = deque(maxlen=1000)
        self.feature_prediction = np.zeros(self.feature_dim)
    ...
    def _init_feature_binding(self):
        ...
        self.feature_binding_weights = np.random.uniform(0.1, 0.3, (self.feature_dim, self.feature_dim))
        self.active_binding_units = []
        self.binding_strengths = deque(maxlen=50)
        self.events: List[Dict] = []
        self.current_event_id = 0
        self.feature_neurons_active = np.zeros(self.feature_dim, dtype=bool)
        self.context_neurons_active = np.zeros(self.feature_dim, dtype=bool)
        self.binding_sync_threshold = 0.7

    def _bind_features(self, feature_spikes: List, context_spikes: List, current_time_ms: float):
        self.feature_neurons_active[:] = False
        self.context_neurons_active[:] = False
        
        for spike in feature_spikes:
            if hasattr(spike, 'channel') and spike.channel < self.feature_dim:
                self.feature_neurons_active[spike.channel] = True
        
        for spike in context_spikes:
            if hasattr(spike, 'channel') and spike.channel < self.feature_dim:
                self.context_neurons_active[spike.channel] = True
        
        feature_active_count = np.sum(self.feature_neurons_active)
        context_active_count = np.sum(self.context_neurons_active)
        
        if feature_active_count > 0 and context_active_count > 0:
            binding_strength = 0.7 + (feature_active_count * context_active_count) / (self.feature_dim ** 2) * 0.3
        else:
        ...
        if binding_strength > self.binding_sync_threshold:
            ...
            event = {
                ...
                'feature_channels': np.where(self.feature_neurons_active)[0].tolist(),
                'context_channels': np.where(self.context_neurons_active)[0].tolist(),
            ...
            self.feature_binding_weights[self.feature_neurons_active, :] += 0.01
            self.feature_binding_weights[:, self.context_neurons_active] += 0.01
            self.feature_binding_weights = np.clip(self.feature_binding_weights, 0.01, 1.0)
        
        return binding_strength

    def _predict_next_feature(self, current_feature: int) -> Tuple[int, float]:
        if 0 <= current_feature < self.feature_dim:
            self.feature_prediction = self.pattern_weights[current_feature]
        else:
            self.feature_prediction = np.zeros(self.feature_dim)
        
        if np.sum(self.feature_prediction) > 0:
            self.feature_prediction = self.feature_prediction / np.sum(self.feature_prediction)
        
        predicted_feature = int(np.argmax(self.feature_prediction))
        prediction_strength = float(self.feature_prediction[predicted_feature])
        
        return predicted_feature, prediction_strength

    def _calculate_prediction_error(self, predicted_feature: int, actual_feature: int) -> float:
        if predicted_feature == actual_feature:
            error = 0.0
        else:
            error = 1.0 - (self.feature_prediction[actual_feature] if 0 <= actual_feature < self.feature_dim else 0)
    ...
    def _learn_pattern(self, feature_sequence: List[int]):
        for feature in feature_sequence:
            if 0 <= feature < self.feature_dim:
                self.feature_counts[feature] += 1
        
        for i in range(len(feature_sequence) - 1):
            from_feature = feature_sequence[i]
            to_feature = feature_sequence[i + 1]
            if 0 <= from_feature < self.feature_dim and 0 <= to_feature < self.feature_dim:
                self.transition_counts[from_feature, to_feature] += 1
                self.transition_history.append((from_feature, to_feature))
        
        total_counts = np.sum(self.feature_counts)
        if total_counts > 0:
            self.central_feature_activation = self.feature_counts / total_counts
        
        total_transitions = np.sum(self.transition_counts)
        if total_transitions > 0:
            self.pattern_weights = self.transition_counts / total_transitions
    ...
    def _detect_pattern(self):
        ...
        normalized_counts = self.feature_counts / np.max(self.feature_counts)
        best_pattern = int(np.argmax(normalized_counts))
        
        self.pattern_strengths[best_pattern] = np.max(normalized_counts)
        self.current_pattern = f"pattern_{best_pattern}"
        self.pattern_confidence = np.max(normalized_counts)
    
    def get_pattern_info(self) -> Dict:
        return {
            'pattern_strengths': self.pattern_strengths.tolist(),
            'current_pattern': self.current_pattern,
            'pattern_confidence': float(self.pattern_confidence),
            'feature_counts': self.feature_counts.tolist(),
            'central_feature_activation': self.central_feature_activation.tolist(),
            'most_active_feature': int(np.argmax(self.feature_counts)) if np.sum(self.feature_counts) > 0 else -1,
        }
    ...
    def _detect_ensembles(self):
        for layer_name in ['pattern_integrator', 'hippocampus']:
            layer_size = self.perceptual_layer_sizes[layer_name]
    ...
    def process_input(self, feature_vector: np.ndarray) -> Dict[str, Any]:
        start_time = time.time()

        encoder_spikes = self.sensory_encoder.process(feature_vector)

        buffer_spikes = []
        current_t = 0.0
        for spike in encoder_spikes:
            t_ms = spike.timestamp * 1000
            while current_t < t_ms:
                current_t += self.dt

            self._apply_feedback_to_perceptual('sensory_buffer', current_t)
            layer_spikes = self.sensory_buffer.process([spike], current_t, self.dt)
            buffer_spikes.extend(layer_spikes)

        extractor_spikes = []
        current_t = 0.0
        for spike in buffer_spikes:
            t_ms = getattr(spike, 'timestamp', 0) * 1000 if getattr(spike, 'timestamp', 0) > 0 else current_t
            while current_t < t_ms:
                current_t += self.dt

            self._apply_feedback_to_perceptual('feature_extractor', current_t)
            layer_spikes = self.feature_extractor.process([spike], [spike], current_t, self.dt)
            extractor_spikes.extend(layer_spikes)

        integrator_spikes = []
        current_t = 0.0
        for spike in extractor_spikes:
            t_ms = getattr(spike, 'timestamp', 0) * 1000 if getattr(spike, 'timestamp', 0) > 0 else current_t
            while current_t < t_ms:
                current_t += self.dt

            self._apply_feedback_to_perceptual('pattern_integrator', current_t)
            layer_spikes = self.pattern_integrator.process([spike], current_t, self.dt)
            integrator_spikes.extend(layer_spikes)

        self._update_ensemble_activity('pattern_integrator', integrator_spikes, current_t)

        hipp_spikes = []
        bg_spikes = []
        current_t = 0.0
        for spike in integrator_spikes:
            t_ms = getattr(spike, 'timestamp', 0) * 1000 if getattr(spike, 'timestamp', 0) > 0 else current_t
            while current_t < t_ms:
                current_t += self.dt
        ...
        cognitive_input = self._generate_cognitive_input(encoder_spikes, buffer_spikes, extractor_spikes)
        ...
        ff_count = self._propagate_all_feedforward(integrator_spikes, hipp_spikes, bg_spikes)
        ...
        if self.learning_enabled:
            ...
            integrator_pattern = [(s.channel, s.timestamp) for s in integrator_spikes[:20]]
            self._update_working_memory(pattern_id, integrator_pattern, current_t)

        fusion_output = self._fuse_outputs(hipp_spikes, bg_spikes, cognitive_spikes)

        perceptual_counts = {
            'sensory_encoder': len(encoder_spikes),
            'sensory_buffer': len(buffer_spikes),
            'feature_extractor': len(extractor_spikes),
            'pattern_integrator': len(integrator_spikes),
        ...
        for spike in integrator_spikes:
            self.spike_history.append(('integrator', spike.channel, current_t))
    ...
    def _generate_cognitive_input(self, encoder_spikes, buffer_spikes, extractor_spikes) -> List[int]:
        input_neurons = set()
        input_size = self.cognitive_layer_sizes['input']

        for spike in encoder_spikes[:30]:
            neuron_id = spike.channel % input_size
            input_neurons.add(neuron_id)

        for spike in buffer_spikes[:25]:
            neuron_id = (spike.channel + 10) % input_size
            input_neurons.add(neuron_id)

        for spike in extractor_spikes[:20]:
    ...
    def _propagate_all_feedforward(self, integrator_spikes, hipp_spikes, bg_spikes) -> int:
        ff_count = 0

        for spike in integrator_spikes:
            for conn in self.cross_connections:
                if (conn.connection_type == ConnectionType.FEEDFORWARD and
                    conn.source_layer == 'pattern_integrator'):

                    source_match = (conn.source_neuron == spike.channel % self.perceptual_layer_sizes['pattern_integrator'])
    ...
    def _inject_feedback_to_perceptual(self, target_layer: str, target_neuron: int, weight: float):
        if target_layer == 'pattern_integrator':
            if hasattr(self.pattern_integrator, 'columns'):
                col_idx = target_neuron % len(self.pattern_integrator.columns)
                col = self.pattern_integrator.columns[col_idx]
            ...
            if hasattr(self.hippocampus, 'neurons') and self.hippocampus.neurons:
                ...
                if hasattr(n, 'I_ext'):
                    n.I_ext += weight * 2.0
        elif target_layer == 'feature_extractor':
            if hasattr(self.feature_extractor, 'neurons') and self.feature_extractor.neurons:
                n_idx = target_neuron % len(self.feature_extractor.neurons)
                n = self.feature_extractor.neurons[n_idx]
                if hasattr(n, 'I_ext'):
                    n.I_ext += weight * 2.0
        elif target_layer == 'sensory_buffer':
            if hasattr(self.sensory_buffer, 'neurons') and self.sensory_buffer.neurons:
                n_idx = target_neuron % len(self.sensory_buffer.neurons)
                n = self.sensory_buffer.neurons[n_idx]
                if hasattr(n, 'I_ext'):
                    n.I_ext += weight * 2.0
    ...
    def _get_stereoscopic_summary(self) -> Dict:
        ...
        return {
            'architecture': 'general_stereoscopic_dual_path_v3',
            'description': '通用双通道立体脉冲神经网络 v3.0 - 基于人脑知识存储机制',
    ...
    def learn_pattern(self, pattern_id: str, feature_vector: np.ndarray):
        self.process_input(feature_vector)
        self.hippocampus.store_sequence(pattern_id)
        seq = self.hippocampus.retrieve_sequence(pattern_id)
        if seq:
            pattern_timestamps = [s.timestamp for s in seq]
            self.basal_ganglia.learn_pattern(pattern_id, pattern_timestamps)
            self.patterns[pattern_id] = pattern_timestamps

    def recall_pattern(self, partial_input: np.ndarray) -> List:
        self.process_input(partial_input)
        recalled = self.hippocampus.retrieve_sequence('last')

        if not recalled and self.patterns:
        ...
        if not recalled:
            integrator_spikes = []
            try:
                results = self.process_input(partial_input)
                integrator_spikes = [s for s in results.get('perceptual_path', {}).get('layers', {}).keys()]
            except:
                pass

            if integrator_spikes:
                partial_pattern = [s % 500 for s in integrator_spikes[:10]]
    ...
    def predict_next_event(self, current_sequence: List[float]) -> Optional[float]:
...
if __name__ == "__main__":
    print("=" * 60)
    print("通用立体脉冲神经网络（General Stereoscopic SNN）v3.0 测试")
    print("=" * 60)

    print("\n初始化通用立体SNN v3.0...")
    stereo_snn = GeneralStereoscopicSNN({
        'feature_dim': 128,
        'num_channels': 128,
        'sparsity': 0.15
    })

    test_feature = np.random.randn(128) * 0.5 + 0.5

    print(f"\n测试特征向量: {len(test_feature)}维")

    print("\n开始通用SNN处理...")
    results = stereo_snn.process_input(test_feature)
    ...
    print("\n=== 测试模式学习 ===")
    stereo_snn.learn_pattern("test_pattern", test_feature)
    state = stereo_snn.get_network_state()
    print(f"  记忆数: {state['memory_count']}")
    print(f"  模式数: {state['patterns']}")
    print(f"  长时记忆项数: {state['state']['long_term_memory_count']}")

    print("\n=== 测试模式回忆 ===")
    partial_feature = test_feature[:64]
    recalled = stereo_snn.recall_pattern(partial_feature)
    print(f"  回忆脉冲数: {len(recalled)}")

    print("\n=== 测试事件预测 ===")
    test_seq = [0.1, 0.3, 0.5, 0.7, 0.9]
    next_t = stereo_snn.predict_next_event(test_seq)
